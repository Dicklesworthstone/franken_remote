...
pub use receive::{ReceiveConfig, ReceivePipeline, ReceivePolicy, ReceiveState, ReceiveUpdate, ReceivedPicture};
...
pub enum DeliveryError {
    Wire(WireError), InvalidPolicy, WrongState, ResourceLimit, AllocationFailed,
    ConflictingPicture, NoncontiguousRecovery, ReferenceExpired, RecoveryExpired,
    ClockRegression, ClockOverflow, StaleGeneration, DecodeFailed, DecodeMismatch,
}
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct MediaBindings { video: u32, recovery: u32, progress: u32, repair: u32 }
impl MediaBindings {
    pub fn new(video: u32, recovery: u32, progress: u32, repair: u32) -> Result<Self, DeliveryError> {
        let values = [video, recovery, progress, repair];
        for (i, value) in values.iter().enumerate() {
            if *value == 0 || values[..i].contains(value) { return Err(DeliveryError::StaleGeneration); }
        }
        Ok(Self { video, recovery, progress, repair })
    }
    pub const fn for_channel(self, channel: Channel) -> u32 {
        match channel { Channel::Video => self.video, Channel::Recovery => self.recovery, Channel::MediaConfig => self.progress, Channel::Control => self.repair }
    }
    pub(crate) fn all_newer_than(self, old: Self) -> bool {
        let highest = old.video.max(old.recovery).max(old.progress).max(old.repair);
        [self.video, self.recovery, self.progress, self.repair].iter().all(|value| *value > highest)
    }
}
...
impl MediaEpoch {
    pub(crate) fn replaces(self, old: Self) -> bool {
        self.configuration > old.configuration || (self.configuration == old.configuration && self.recovery > old.recovery)
    }
}
pub(crate) fn deadline(now: u64, duration: u64) -> Result<u64, DeliveryError> {
    now.checked_add(duration).ok_or(DeliveryError::ClockOverflow)
}
