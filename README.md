...
| `frd run --software-explicit [--headless]` | Hosts one X11 display (or a private cookie-authenticated Xvfb) to native viewers: tailnet-bound listener, firewall ingress rule, Tailscale WhoIs admission (own-user or tailnet scope), capture worker launched on admission, software HEVC, renewal, fixed-order cleanup | namespace e2e: viewers receive frames past the initial lease, sequential viewers and departing inspections are served without counting as host failures, a silent viewer expires, host faults stop it with a typed error, clean stop. **Live host side** (root, real tailnet): obtains the Tailscale certificate, installs and verifies the `tailscale0` firewall rule, listens, stops on Ctrl-C and removes the rule. No live client yet |
| `fr displays NODE --experimental-native` | One authenticated connection that fetches the host's display catalog | namespace e2e against `frd run` |
| `fr connect NODE --view-only --experimental-native --display only` | Native X11 viewer: software HEVC decode in a seccomp-sandboxed worker, `XPutImage` presentation, reconnects | namespace e2e against `frd run` with **real media**: the real capture worker encodes a host Xvfb in software HEVC, `fr connect` decodes and presents it on a second Xvfb, and pixels (initial and changed desktop) are verified by an independent X client. The host pointer (excluded from capture) is forwarded as a reliable shape plus replaceable positions and composited as one client-rendered overlay; the same namespace e2e reads its pixels back at the mapped position, after a host pointer move and after a host cursor-image change (Xvfb only: not evidence for a GPU compositor, HiDPI scaling or Wayland). Not yet on a live tailnet |
| `frd run --software-explicit --input-agent PATH` + `fr connect NODE --control --experimental-native --display only` | Opt-in remote control: the first viewer gets an exclusive controlled share (later viewers are refused `Busy`). The client requests control once, after its window shows a fresh frame, then captures keys, buttons and the absolute pointer from its window. The host executes them with XTest in a per-lease `fr-input-agent` child that shows a mandatory "Stop remote control" indicator; `frd` itself never loads Xlib. No local approval prompt: this runs only under the operator's explicit `approval none` choice | namespace e2e with **real media and real input**: XTest input on the viewer display reaches the host display, where an independent X client observes the exact pointer position and non-`SendEvent` ButtonPress/KeyPress events. Planted negatives: freezing the client past its 3 s lease ends the executor and later input has no host effect; an XTest click on the host indicator does not revoke; a host without an input agent refuses with `host_control_unavailable`. The host cursor is forwarded in the controlled share too, with ONE renderer: the controller's local pointer carries the host's confirmed shape (read back from the viewer display with `XFixesGetCursorImage`) while the host follows its input, and the composited overlay at the confirmed position, with the local pointer blanked, when the host moves its own pointer or the local pointer leaves the window; planted negatives (host never sends the shape; client ignores cursor records while controlling) fail that e2e. Not yet on a live tailnet; Xvfb only (no compositor, HiDPI or Wayland claim), no physical input devices |
| `frd run ... --input-agent PATH --clipboard` + `fr connect NODE --control --clipboard ...` | Opt-in UTF-8 text clipboard for the controller, both directions, on the existing bounded begin/chunk/commit records (items up to 1 MiB). It exists only while the controller's input lease is active: offered after the grant, attached under the input attachment, fenced by revoke/expiry. On the host the X11 CLIPBOARD is owned by a per-lane `fr-input-agent --clipboard` child (`frd` still loads no X11); the client's owner runs in `fr`. Both sides must opt in; otherwise control is unchanged and the client's completion reports `clipboard_active: false` with a typed `clipboard_absence` | namespace e2e with **real X11 selections**: an independent X client that copies on the viewer display is pasted by an independent X client on the host display, and the reverse, with unique non-ASCII texts. Planted negatives: a host child that acknowledges but never sets the selection, and a client that never sends its local copies, both fail it; a host without `--clipboard` keeps control and reports `host_clipboard_unavailable`; freezing the client past its lease ends the clipboard child and later copies cross neither way. Xvfb selections in a namespace: not a desktop clipboard manager, not Wayland, not a live tailnet. Text only (no images) |
| `frd run --software-explicit --audio [--audio-server SOCKET] [--audio-sink NAME]` + `fr connect NODE --view-only --audio --experimental-native --display only` | Opt-in host playback audio (off by default on both ends). The host offers the optional `native-audio-down` capability only with `--audio`; a view-only viewer that asked for it gets its own `audio-down` channel (reliable configuration/stop lane plus `AudioPacket` datagrams) after the same admission that gates pixels. A separate `fr-media-worker --audio` child records the PulseAudio monitor of the selected (default) sink and encodes Opus (48 kHz stereo, 20 ms); `frd` links neither libpulse nor libopus. Packets start only after the viewer's `AudioConfigured`, stale audio is dropped at a bounded ring (count and bytes), and each viewer has its own audio epochs. The client plays through the local PulseAudio server and reports `audio_active`/`audio_absence` in its completion record; `--control --audio` is refused (`audio_control_unsupported`) | namespace e2e with **real audio**: an independent player writes a tone into a private PulseAudio null sink on the host, the shipped `fr connect --view-only --audio` plays into a second private PulseAudio server, and an independent recorder detects the tone (Goertzel) and a frequency change on the client sink's monitor, sustained over 2 s. Planted negatives: a host that never sends packets, and a client that drops packets before playback, both fail that test. A host without `--audio` yields `audio_absence: host_did_not_offer` and no tone; a missing monitor yields `host_audio_unavailable` while video continues. Private null sinks only: not a desktop PipeWire session, speakers, audibility or a live tailnet |
| `frd run ... --input-agent PATH --files DIR [--files-max-file-bytes N] [--files-max-session-bytes N]` + `fr connect NODE --control --send PATH [--send PATH]...` | Opt-in viewer-to-host file sending (off by default on both ends). `DIR` must be an existing absolute directory owned by the user running `frd`, not group/other-writable and reached without symlinks; it is pinned by descriptor at startup (typed refusal otherwise). The controller's explicitly selected regular files (up to 8; symlinks, directories and special files refused by type before connecting) travel in order over the existing one-use `native-file-receive` + `file-channel-scope` lane with the ATP full-object profile. The lane is offered only on the controller's own session after its grant and first lease renewal, and attaches only under the active input attachment; the host's bounded disk worker checks the lease before every write and immediately before publication, stages privately and publishes with an atomic no-replace rename. Per-file and per-session (cumulative declared) byte limits are enforced before any byte is written. A taken name is refused (`host_conflict`), never overwritten. The completion reports each file by index and size only (`files_sent`, `files_refused` with typed reasons, `files_absence`). Not together with `--clipboard` in this build | namespace e2e with **real files**: a 3 MiB random file with a non-ASCII name lands under exactly that name with an identical SHA-256; a second file whose name the host already holds is refused `host_conflict` and the original bytes are unchanged; freezing the client mid-transfer past its lease removes the private staging file and nothing ever appears under the final name; a host without `--files` keeps control and reports `host_files_unavailable`. Planted negatives: a host that writes zeros and a client that never starts the transfer both fail it. Viewer to host only; no resumption, directories, downloads, synchronization, picker or drag-and-drop UI; not a live tailnet |
| `frd ingress-helper [--config PATH]` (root, system unit) | Lets an unprivileged `frd run` (for example a systemd user unit) enforce its ingress rule: a root helper whose whole API is install/renew/remove of one drop-only nftables rule per connection for the configured tailnet interface, an address the kernel reports on it and a nonzero port. Callers are admitted by `SO_PEERCRED` against a root-owned uid list; a rule lives exactly as long as its connection; `frd install --user` prints the helper unit and config with `[root]` steps | real nftables in a private network namespace (TUN named `tailscale0` plus a veth peer): UDP/TCP dropped via the non-tailnet interface and passed via the tailnet one, disallowed uid refused, rule removed on broker SIGKILL, helper-crash reclaim. Not yet on a live tailnet host; the emitted systemd unit has not run under a live systemd |
| `fr robot ...`, `fr disconnect` | Refuse with typed errors (`robot_surface_unavailable`, `no_background_session`) | unit/CLI tests |

Not available: local approval prompts in `frd run` (control therefore requires the operator's explicit input-agent opt-in and runs unattended with its indicator), text, pixel-scroll and relative-pointer input (discrete wheel scrolling is forwarded, with native capture and XTest wheel tests but not yet in the control e2e), observers alongside a controller or observer-to-controller handoff, audio for a controlling session, microphone (client-to-host) audio, host-to-viewer file transfer, directory/resumed/synchronized transfers and file sending alongside the clipboard, image clipboard and any clipboard for observers, hardware HEVC selection, Wayland, macOS, Windows, browser and mobile clients. Earlier documents that marked GNOME/KDE/Hyprland, Windows GPU rows or Windows/macOS worker sandboxes as passed or enforced were withdrawn on 2026-09-24: no evidence existed.
...
### Bounded fragment repair

Selective repair covers every missing fragment even when disjoint holes exceed
its negotiated range count. It coalesces the smallest already-received gaps
instead of silently omitting later holes, minimizing duplicate fragments within
the existing range limit. Sender byte/rate limits, retry counts and the original
receiver reference deadline still bound recovery. The `fr-media` repair-coverage
tests exercise actual wire requests and sender/receiver round trips, including
