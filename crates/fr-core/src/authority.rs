...
/// Diagnostics expose state, never live challenge or ticket material.
/// There is deliberately no `Clone` implementation for a live authority owner.
///
/// ```compile_fail
/// use fr_core::authority::{AuthorityPolicy, SessionAuthority};
/// use fr_core::ids::RemoteSessionId;
/// let owner = SessionAuthority::new(
///     RemoteSessionId::from_raw(1), AuthorityPolicy::plan_defaults());
...
impl fmt::Debug for SessionAuthority {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("SessionAuthority")
            ...
            .field("has_ticket", &self.lease.is_some_and(|lease| lease.ticket.is_some()))
            .field("observation_challenge_pending", &self.observation_challenge.is_some())
            .field("control_challenge_pending", &self.control_challenge.is_some())
            .field("last_checked", &self.last_checked)
            .field("clock_faulted", &self.clock_faulted)
...
mod tests {
    ...
    #[test]
    fn diagnostic_output_does_not_depend_on_live_credentials() {
        fn with_secrets(secret: u128) -> SessionAuthority {
            let (mut owner, old_lease, _) = viewing_with_control(at(0));
            owner.revoke_lease();
            let lease = InputLeaseId::from_raw(secret);
            assert_ne!(lease, old_lease);
            owner.grant_lease(lease, at(0)).unwrap();
            owner
                .issue_input_ticket(lease, InputTicketId::from_raw(secret), at(0))
                .unwrap();
            owner.issue_observation_challenge(secret, at(0)).unwrap();
            owner.issue_control_challenge(secret, at(0)).unwrap();
            owner
        }
        let first = with_secrets(0x1234_5678_9abc_def0);
        let second = with_secrets(0xfedc_ba98_7654_3210);
        assert_eq!(format!("{first:?}"), format!("{second:?}"));
        assert_eq!(format!("{first:#?}"), format!("{second:#?}"));
        assert_eq!(format!("{:?}", Some(&first)), format!("{:?}", Some(&second)));
        assert!(format!("{first:?}").contains("has_ticket: true"));
        assert!(format!("{first:#?}").len() < 1_024);
    }
}
