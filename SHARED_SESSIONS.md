...
Five new tests use actual TLS/UDP session negotiation, completed media attachments,
...
revocation before further media admission. Eleven existing persistent-session
renewal/refresh regressions also pass. Test identity, encoded parameters and
decode completions are explicit fixtures, not live-tailnet or HEVC/GPU evidence.
...
uses a separate source copy with only these sixteen tests registered, preserving
...
its original failed scheduling assertion is retained in local logs. No full
workspace, cold dependency rebuild, hardware or live-tailnet pass is claimed.
