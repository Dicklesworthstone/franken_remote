...
fn check_bounds(video: &Video, counts: &mut RecoveryCounts) {
    let (send, receive) = (
        video.subscription.cache_usage(),
        video.receiver.budget_usage(),
    );
    assert!(
        send.bytes <= SendPolicy::default().max_cached_bytes
            && send.pictures <= SendPolicy::default().max_cached_pictures
    );
    assert!(
        u64::try_from(receive.bytes).unwrap()
            <= video.wire.protocol().per_viewer_compressed_bytes()
            && receive.pictures <= usize::from(video.wire.protocol().reassembly_window_pictures())
    );
