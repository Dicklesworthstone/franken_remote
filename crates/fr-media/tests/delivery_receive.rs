...
fn config() -> ReceiveConfig {
    ReceiveConfig {
        ...
        epoch: MediaEpoch { configuration: CodecConfigurationGeneration::INITIAL, recovery: RecoveryGeneration::INITIAL },
        policy: ReceivePolicy::default(),
    }
}
fn payload() -> Vec<u8> { (0_u8..=255).cycle().take(2_500).collect() }
fn descriptor(frame: u64, reference: Option<u64>) -> FrameDescriptor {
    FrameDescriptor { frame, reference, total_bytes: 2_500, stride: 1_077, capture_micros: 123 }
}
fn fragments(d: FrameDescriptor, bytes: &[u8], c: ReceiveConfig) -> Vec<Vec<u8>> {
    (0..d.fragment_count().unwrap()).map(|index| {
        let mut packet = vec![0; c.limits.record_bytes()];
        let n = encode_fragment(Fragment { descriptor: d, index, bytes: &bytes[d.fragment_range(index).unwrap()] }, c.bindings.for_channel(Channel::Video), &c.limits, &mut packet).unwrap();
        packet.truncate(n); packet
    }).collect()
}
fn recovery(receiver: &mut ReceivePipeline, c: ReceiveConfig, now: u64) {
    ...
    for (index, chunk) in bytes.chunks(1_077).enumerate() {
        ...
        let n = encode_recovery(RecoveryChunk { frame: 0, total_bytes: 2_500, offset: u32::try_from(index * 1_077).unwrap(), capture_micros: 123, bytes: chunk }, c.bindings.for_channel(Channel::Recovery), &c.limits, &mut packet).unwrap();
        receiver.receive(Channel::Recovery, &packet[..n], now).unwrap();
    }
}
fn running(c: ReceiveConfig) -> (ReceivePipeline, MediaBudget) {
    ...
    receiver.decoder_configured(0).unwrap(); recovery(&mut receiver, c, 0);
    let picture = receiver.take_decodable(0).unwrap().unwrap();
    assert_eq!(picture.bytes(), payload());
    receiver.acknowledge_decode(&picture, true, 0).unwrap(); drop(picture);
    (receiver, budget)
}
fn progress(d: FrameDescriptor, c: ReceiveConfig) -> Vec<u8> {
    let mut packet = vec![0; c.limits.record_bytes()];
    let n = encode_progress(Progress { descriptor: d, observed_micros: 123, observation: SourceObservation::Captured, pipeline: PipelineState::Idle }, c.bindings.for_channel(Channel::MediaConfig), &c.limits, &mut packet).unwrap();
    packet.truncate(n); packet
}

#[test]
fn configured_recovery_and_decode_are_distinct_non_circular_milestones() {
    let c = config(); let budget = MediaBudget::new(c.limits.protocol()).unwrap();
    let mut receiver = ReceivePipeline::new(c, budget.clone()).unwrap();
    assert_eq!(receiver.state(), ReceiveState::AwaitingConfiguration);
    let packets = fragments(descriptor(1, Some(0)), &payload(), c);
    assert_eq!(receiver.receive(Channel::Video, &packets[0], 0), Err(DeliveryError::WrongState));
    ...
    for packet in &packets { receiver.receive(Channel::Video, packet, 2).unwrap(); }
...
fn out_of_order_duplicates_reassemble_once_without_extra_budget() {
    let c = config(); let (mut receiver, budget) = running(c);
    let bytes = payload(); let packets = fragments(descriptor(1, Some(0)), &bytes, c);
    receiver.receive(Channel::Video, &packets[2], 1).unwrap();
    let reserved = budget.usage();
    assert_eq!(receiver.receive(Channel::Video, &packets[2], 2), Ok(ReceiveUpdate::Duplicate));
    assert_eq!(budget.usage(), reserved);
    receiver.receive(Channel::Video, &packets[0], 3).unwrap();
    assert!(receiver.take_decodable(3).unwrap().is_none());
    assert_eq!(receiver.receive(Channel::Video, &packets[1], 4), Ok(ReceiveUpdate::PictureComplete));
    ...
    assert_eq!(receiver.receive(Channel::Video, &packets[0], 5), Ok(ReceiveUpdate::Obsolete));
    assert_eq!(budget.usage(), reserved);
    drop(picture); assert_eq!(budget.usage(), BudgetUsage::default());
}
#[test]
fn conflicting_duplicate_fences_the_chain_and_frees_pending_buffers() {
    ...
    *packets[0].last_mut().unwrap() ^= 0xff;
    assert_eq!(receiver.receive(Channel::Video, &packets[0], 2), Err(DeliveryError::ConflictingPicture));
...
fn changed_descriptor_is_not_a_second_picture_with_the_same_id() {
    let c = config(); let (mut receiver, _) = running(c);
    ...
    assert_eq!(receiver.receive(Channel::Video, &b[1], 2), Err(DeliveryError::ConflictingPicture));
}
#[test]
fn stale_for_display_reference_still_unlocks_fresh_dependent_picture() {
    ...
    for packet in &second { receiver.receive(Channel::Video, packet, 50_000).unwrap(); }
    assert!(receiver.take_decodable(50_000).unwrap().is_none());
    for packet in &first[1..] { receiver.receive(Channel::Video, packet, 60_000).unwrap(); }
    ...
    receiver.acknowledge_decode(&reference, true, 61_000).unwrap(); drop(reference);
    let latest = receiver.take_decodable(61_000).unwrap().unwrap();
    assert!(latest.within_display_queue_budget()); assert_eq!(latest.descriptor().frame, 2);
}
#[test]
fn reliable_announcement_detects_an_entirely_lost_final_frame() {
    let c = config(); let (mut receiver, _) = running(c);
    let d = descriptor(1, Some(0)); let announcement = progress(d, c);
    receiver.receive(Channel::MediaConfig, &announcement, 100).unwrap();
    assert_eq!(receiver.next_deadline(), Some(20_100));
    let mut packet = [0; 1_150];
    assert_eq!(receiver.repair_request(20_099, &mut packet), Ok(None));
    let n = receiver.repair_request(20_100, &mut packet).unwrap().unwrap();
    let request = decode_repair(Record::decode(&packet[..n], &c.limits, c.bindings.for_channel(Channel::Control), Channel::Control).unwrap(), 3, &c.limits).unwrap();
    assert_eq!(request.frame, 1);
    assert_eq!(request.ranges().collect::<Vec<_>>(), [RepairRange { start: 0, end: 3 }]);
    assert_eq!(receiver.repair_request(20_101, &mut packet), Ok(None));
    receiver.receive(Channel::MediaConfig, &announcement, 240_000).unwrap();
...
fn repair_requests_select_only_missing_fragments_and_stop_at_retry_limit() {
    ...
    for now in [20_000, 80_000, 140_000] {
        ...
        let request = decode_repair(Record::decode(&out[..n], &c.limits, c.bindings.for_channel(Channel::Control), Channel::Control).unwrap(), 3, &c.limits).unwrap();
        assert_eq!(request.ranges().collect::<Vec<_>>(), [RepairRange { start: 0, end: 1 }, RepairRange { start: 2, end: 3 }]);
    }
...
fn stale_bindings_allocate_nothing_and_do_not_break_current_stream() {
    ...
    assert_eq!(receiver.receive(Channel::Video, &packet, 1), Err(DeliveryError::StaleGeneration));
...
fn recovery_refuses_gaps_and_never_exposes_a_partial_idr() {
    let c = config(); let budget = MediaBudget::new(c.limits.protocol()).unwrap();
    let mut receiver = ReceivePipeline::new(c, budget.clone()).unwrap(); receiver.decoder_configured(0).unwrap();
    let mut packet = [0; 1_150];
    let n = encode_recovery(RecoveryChunk { frame: 0, total_bytes: 2_500, offset: 1, capture_micros: 123, bytes: &[1, 2] }, c.bindings.for_channel(Channel::Recovery), &c.limits, &mut packet).unwrap();
    assert_eq!(receiver.receive(Channel::Recovery, &packet[..n], 1), Err(DeliveryError::NoncontiguousRecovery));
    assert_eq!(budget.usage(), BudgetUsage::default());
...
fn held_decoder_inputs_remain_charged_after_close_and_generation_replacement() {
    let mut c = config();
    let limits = ProtocolLimits::with_overrides(LimitOverrides { reassembly_window_pictures: Some(2), ..LimitOverrides::default() }).unwrap();
    ...
    assert_eq!(old_usage.pictures, 1); assert!(old_usage.bytes > old.bytes().len());
    let epoch = MediaEpoch { recovery: RecoveryGeneration::from_raw(1), ..c.epoch };
    let bindings = MediaBindings::new(5, 6, 7, 8).unwrap();
    receiver.replace(epoch, bindings, 2).unwrap();
    assert_eq!(budget.usage(), old_usage);
    assert_eq!(receiver.acknowledge_decode(&old, true, 2), Err(DeliveryError::StaleGeneration));
    receiver.close(); assert_eq!(budget.usage(), old_usage);
    drop(old); assert_eq!(budget.usage(), BudgetUsage::default());
}
#[test]
fn picture_count_includes_completed_but_not_released_decoder_inputs() {
    let mut c = config();
    let p = ProtocolLimits::with_overrides(LimitOverrides { reassembly_window_pictures: Some(2), ..LimitOverrides::default() }).unwrap();
    c.limits = MediaLimits::new(p, 1_150, 16_384, 64).unwrap();
    let budget = MediaBudget::new(&p).unwrap();
    let mut receiver = ReceivePipeline::new(c, budget.clone()).unwrap(); receiver.decoder_configured(0).unwrap();
    recovery(&mut receiver, c, 0);
    let idr = receiver.take_decodable(0).unwrap().unwrap(); receiver.acknowledge_decode(&idr, true, 0).unwrap();
    for packet in fragments(descriptor(1, Some(0)), &payload(), c) { receiver.receive(Channel::Video, &packet, 1).unwrap(); }
    let next = receiver.take_decodable(2).unwrap().unwrap(); receiver.acknowledge_decode(&next, true, 2).unwrap();
    assert_eq!(budget.usage().pictures, 2);
    let packets = fragments(descriptor(2, Some(1)), &payload(), c);
    assert_eq!(receiver.receive(Channel::Video, &packets[0], 3), Err(DeliveryError::ResourceLimit));
    drop(idr); drop(next); assert_eq!(budget.usage(), BudgetUsage::default());
}
#[test]
fn decode_failure_never_promotes_dependents_and_idle_timer_ends_recovery() {
    ...
    assert_eq!(receiver.acknowledge_decode(&idr, false, 2), Err(DeliveryError::DecodeFailed));
    assert_eq!(receiver.state(), ReceiveState::NeedsRecovery);
    assert_eq!(budget.usage().pictures, 1); drop(idr);
    let mut other = ReceivePipeline::new(c, budget).unwrap(); other.decoder_configured(0).unwrap();
...
fn clock_faults_close_and_binding_or_epoch_reuse_cannot_reopen() {
    ...
    assert_eq!(receiver.replace(c.epoch, c.bindings, 101), Err(DeliveryError::WrongState));
    let (mut fresh, _) = running(c);
    assert_eq!(fresh.replace(c.epoch, c.bindings, 0), Err(DeliveryError::StaleGeneration));
    let budget = MediaBudget::new(c.limits.protocol()).unwrap();
    let mut overflow = ReceivePipeline::new(c, budget).unwrap();
    assert_eq!(overflow.decoder_configured(u64::MAX), Err(DeliveryError::ClockOverflow));
