...
- **Enforced sandbox boundaries**: Where supported by the OS and compatible with the media pipeline role, workers operate under kernel-enforced privilege reduction:
  ...
  - *Linux host capture/encode worker*: **no kernel sandbox**. It runs as a separate same-user process with a cleared environment (only `DISPLAY`/`XAUTHORITY`) and its two pipes; process separation is crash/hang containment only. The same applies to its `--audio` role (PulseAudio monitor capture and Opus encode).
  - *Linux client Opus decoder*: **not isolated**. Host playback audio is decoded by libopus inside the `fr` client process on its view-only session thread, after record, epoch and negotiated packet/sample bounds checks. Unlike the HEVC decoder, a libopus defect is exposed to host-generated packets without process isolation.
